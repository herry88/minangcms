    ...
    var ns = $('ol.sortable').nestedSortable({
		...
		update: function(){			
			...
			$.ajax({
				type:"post",
				dataType:"json",
				url:'<?=site_url(roleURIUser());?>/style/menus/reorder?menu=bottom',
				data:serialized,
				success:function(x){
					getMenu();					
				}
...
function savePage(menuid,menuparent){	
	...
	$.ajax({
		...
		data:'title='+pageTitle+"&menu="+menuid+"&pos=bottom&parent="+menuparent,
		success:function(){
			getMenu();
		}
	});
...
function saveCategory(menuid,menuparent){	
	...
	$.ajax({
		...
		data:'title='+catTitle+"&menu="+menuid+"&pos=bottom&parent="+menuparent,
		success:function(){
			getMenu();
		}
	});
...
function saveLink(menuid,menuparent){	
	...
	$.ajax({
		...
		data:'title='+linkTitle+"&url="+linkUrl+"&menu="+menuid+"&pos=bottom&parent="+menuparent,
		success:function(){
			getMenu();
		}
	});
