...
class Events extends MX_Controller
{
	...
	function addapply(){
		...
		if($this->m_security->startPost()==TRUE){
			...
			$tglevent=$this->input->post('tanggalevent');
			...
			if($tipe=="post"){
				...
				$proses=$this->pm->addEvent($judul,$datetime,$konten,$status,$tags,$keyword,$komen,$tampilan,$tglevent,$jamevent);
			}
			
			if($proses['status']==TRUE){
				$lastid=$proses['lastid'];
				$this->session->set_flashdata('info','<div class="alert alert-info">Berhasil menambah event</div>');
				redirect(base_url(roleURIUser().'content/events/edit').'?id='.$lastid,'refresh');
			}else{
				redirect(base_url(roleURIUser().'content/events/add'),'refresh');
			}
	...
	function edit(){
		...
		if($this->m_database->isBOF('posts',$s)==FALSE){
			...
			redirect(base_url(roleURIUser().'content/events'),'refresh');
		}
	...
	function editapply(){
		...
		if($this->m_security->startPost()==TRUE){
			...
			if($tipe=="post"){
				...
				$proses=$this->pm->editEvent($postid,$judul,$datetime,$konten,$status,$tags,$keyword,$komen,$tampilan,$tglevent,$jamevent);
			}
			
			if($proses['status']==TRUE){				
				$this->session->set_flashdata('info','<div class="alert alert-info">Berhasil mengubah event</div>');
				redirect(base_url(roleURIUser().'content/events/edit').'?id='.$postid,'refresh');
			}else{
				redirect(base_url(roleURIUser().'content/events/add'),'refresh');
			}
